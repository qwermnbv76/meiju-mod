// MeijuMod v2.9.2 - RTC voice + cleanup (no unlimited dating)
(function () {
    "use strict";
    var V = "2.9.2";
    ...
    function defCfg() { return { ver: V, on: true, login: true, pet: true, ocr: true, scene: true, rtc: true }; }
    ...
    function getRtcCfg() { var c = loadApiCfg(K_RTC); return { appId: c.appId || "", appKey: c.appKey || "" }; }
    ...
    // ============ DESKTOP PET ============
    if (M.pet) {
        function injectPetApiToManager(mgr) { if (!mgr) return; var a = getPetApi(); if (a && a.apiKey && a.baseurl) { mgr.apiKey = a.apiKey; mgr.baseURL = a.baseurl; mgr.modelName = a.modelname; mgr.isServerMode = false; } }
...
// ============ RTC VOICE ============
    if (M.rtc) {
        ...
        poll(function() { return !!(window.RealtimeVoiceManager && window.RealtimeVoiceManager.prototype.init); }, function() {
            ...
            window.RealtimeVoiceManager.prototype.init = async function() {
                var c = getRtcCfg();
                if (c.appId) { this.config.appId = c.appId; }
                if (c.appKey) { this.config.appKey = c.appKey; }
                return origInit.call(this);
            };
    ...
    function buildRtcSection(d) {
        ...
        var i1 = document.createElement("input"); i1.type = "text"; i1.id = "mod-rtc-appid"; i1.className = "mod-api-input"; i1.placeholder = "\u706b\u5c71\u5f15\u64ce\u5e94\u7528ID"; i1.value = rtc.appId || ""; fg1.appendChild(i1); d.appendChild(fg1);
        ...
        var i2 = document.createElement("input"); i2.type = "password"; i2.id = "mod-rtc-appkey"; i2.className = "mod-api-input"; i2.placeholder = "\u706b\u5c71\u5f15\u64ce\u5bc6\u94a5"; i2.value = rtc.appKey || ""; fg2.appendChild(i2); d.appendChild(fg2);
        var br = document.createElement("div"); br.style.cssText = "display:flex;gap:8px;margin-bottom:6px;";
        var b1 = document.createElement("button"); b1.className = "mod-api-btn secondary"; b1.textContent = "\u6d4b\u8bd5\u8fde\u63a5"; b1.onclick = function() { testRtc(); };
    ...
    function saveRtc() {
        var v = readRtcInputs();
        saveApiCfg(K_RTC, { appId: v.appId.trim(), appKey: v.appKey.trim() });
    ...
    function buildPanel() {
        ...
        setTimeout(function() {
            var keys = ["login", "pet", "ocr", "scene", "rtc"], ids = ["m-login", "m-pet", "m-ocr", "m-scene", "m-rtc"];
    ...
    var panelTimer = setInterval(function() { var modal = document.getElementById("ai-settings-modal"); if (modal && modal.style.display !== "none" && !document.getElementById("mod-panel")) injectPanel(); }, 500); setTimeout(function() { clearInterval(panelTimer); }, 60000);
